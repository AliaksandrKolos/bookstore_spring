...
<c:if test="${books != null && !books.isEmpty()}">
    ...
    <div class="pagination">
        <a href="${pageContext.request.contextPath}/books/search_title?page=0&size=${param.size}&title=${param.title}">
            <spring:message code="books.first"/>
        </a>

        <a href="${pageContext.request.contextPath}/books/search_title?page=${page <= 0 ? 0 : page - 1}&size=${param.size}&title=${param.title}">
            <spring:message code="books.prev"/>
        </a>

        <c:out value="${page + 1}"/>

        <a href="${pageContext.request.contextPath}/books/search_title?page=${page < totalPages - 1 ? page + 1 : totalPages - 1}&size=${param.size}&title=${param.title}">
            <spring:message code="books.next"/>
        </a>

        <a href="${pageContext.request.contextPath}/books/search_title?page=${totalPages - 1}&size=${param.size}&title=${param.title}">
